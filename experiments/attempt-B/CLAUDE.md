...
- UI: `ui.js`. DOM writes only. Imports no core module other than paytable.js for rendering.
...
- JSDoc type annotations on all exported functions. Shared types live in `src/types.js`.
- Small functions (around 30 lines or fewer); extract helpers when a function grows.
...
- ESLint, stylelint, and html-validate. All three MUST pass before declaring any module done.
...
Do not reference or modify any file outside `experiments/attempt-B/`. The parent `plan/`, the sibling `experiments/attempt-A/`, and the repo-root `src/` are off-limits to every turn.

## Out of Scope (Game Features)

Scatter, Free Spins (proper bonus mode), multipliers, cascading, auto-spin, both-ways pay, session history log, and accounts. If a prompt asks for these, reject and point to §10 of `SPEC.md`.
